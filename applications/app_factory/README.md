...
The longer-term vision is a fully automated path from questionnaire submission to a live, deployed application. The generated blueprints would be fed into the existing AVA deployment pipeline — the same Step Functions / CodeBuild infrastructure used for reference implementations — to scaffold, build, and deploy the application in the customer's AWS account without any manual engineering work.
...
- [Reference Implementations](../reference_implementations/) — End-to-end full-stack solutions
