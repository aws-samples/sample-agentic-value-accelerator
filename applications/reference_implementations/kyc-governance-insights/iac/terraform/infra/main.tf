terraform {
  ...
  # Partial backend configuration — bucket, key, region and the lock table are
  # injected at init time by deploy_agent.sh, which reads $STATE_BUCKET/$LOCK_TABLE
  # from the AVA CodeBuild pipeline:
  #   terraform init -backend-config="bucket=$STATE_BUCKET" \
  #                  -backend-config="key=<prefix>/infra/terraform.tfstate" \
  #                  -backend-config="dynamodb_table=$LOCK_TABLE"
  # The key is injected rather than fixed here because it is framework-scoped.
  # Remote state is what lets a redeploy adopt the running deployment instead of
  # starting from an empty state and colliding with existing resource names.
  backend "s3" {
    encrypt = true
  }
}
...
locals {
  ...
  use_case_short_map = {
    ...
    "kyc_banking"              = "kyc"
    "kyc_governance_insights"  = "kycgov"
