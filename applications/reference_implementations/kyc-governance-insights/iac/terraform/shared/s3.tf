...
locals {
  ...
  # This module lives at iac/terraform/shared, so the application root — which owns
  # data/samples — is three levels up. Getting this depth wrong is silent: the
  # fileset() below simply yields nothing, no sample data is uploaded, and an
  # existing deployment's sample objects are planned for deletion instead.
