terraform {
  ...
  # Partial backend configuration — see infra/main.tf. Without a backend block here,
  # -backend-config is silently ignored and Terraform falls back to local state, so
  # deploy.sh could not read this module's ui_bucket_name / cloudfront_distribution_id
  # outputs to publish the console bundle.
  backend "s3" {
    encrypt = true
  }
}
...
locals {
  ...
  # /svc/* is only wired when the console gateway has been deployed.
  svc_enabled = var.svc_gateway_domain != ""
}

# The x-origin-verify value proves to the gateway's authorizer that a request came
# through this distribution. Read at apply time so the secret is never passed in as a
# variable and never lands in a tfvars file.
...
resource "aws_cloudfront_distribution" "ui" {
  ...
  # Origin 3: the consolidated console gateway serving /svc/*. Previously attached
  # by phase2 scripts outside Terraform; owned here so the auth function below is
  # actually applied to it and so an apply cannot delete it.
  dynamic "origin" {
    for_each = slice(["svc-gateway"], 0, local.svc_enabled ? 1 : 0)
    content {
      domain_name = var.svc_gateway_domain
  ...
  # /svc/*: the governance services (Cedar, registry, HITL, grounding, validator,
  # metrics, policy-config) behind the consolidated gateway. Same edge auth as
  # /api/*: without it these APIs answer anonymous requests, since the gateway's
  # own authorizer only proves the call arrived via CloudFront — CloudFront adds
  # that header for anonymous callers too.
  dynamic "ordered_cache_behavior" {
    ...
    content {
      ...
      forwarded_values {
        query_string = true
        headers      = ["Content-Type", "Accept", "Origin", "x-tenant-id"]
        # ava_session must reach the function, or every call is treated as anonymous.
        cookies { forward = local.auth_enabled ? "all" : "none" }
      }

      dynamic "function_association" {
        # Same slice() workaround as /api/* — a ternary between toset([...]) and
        # toset([]) fails type unification on Terraform 1.9.
        for_each = slice(["viewer-request"], 0, local.auth_enabled ? 1 : 0)
        content {
          event_type   = function_association.value
          function_arn = aws_cloudfront_function.jwt_auth[0].arn
