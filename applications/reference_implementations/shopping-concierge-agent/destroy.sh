...
# Sanitize identifier the same way deploy.sh does so stack/parameter names match.
AMPLIFY_ID=$(echo "$DEPLOY_NAME" | sed 's/[-_]//g' | tr '[:upper:]' '[:lower:]' | cut -c1-14)
echo "Destroy target: $DEPLOY_NAME (deployment id: $AMPLIFY_ID) in $AWS_REGION"
...
npx cdk destroy "FrontendStack-${AMPLIFY_ID}" --force 2>&1 || echo "Warning: Frontend stack destroy failed or already deleted"
cd ../..

# Step 2: Agent (Runtime + Gateway + Memory)
echo "=== Step 2/4: Destroying agent stack ==="
cd infrastructure/agent-stack && npm install
npx cdk destroy "AgentStack-${AMPLIFY_ID}" --force 2>&1 || echo "Warning: Agent stack destroy failed or already deleted"
cd ../..
...
AMPLIFY_STACK=$(aws cloudformation list-stacks --stack-status-filter CREATE_COMPLETE UPDATE_COMPLETE \
  --query "StackSummaries[?starts_with(StackName, \`amplify-sampleconciergeagent-${AMPLIFY_ID}-sandbox\`) && !contains(StackName, \`NestedStack\`)].StackName | [0]" \
